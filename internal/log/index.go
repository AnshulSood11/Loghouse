...
func newIndex(f *os.File, c Config) (*index, error) {
	...
	if idx.mmap, err = gommap.Map(
		idx.file.Fd(),
		gommap.PROT_READ|gommap.PROT_WRITE, //  Pages may be read. and Pages may be written
		gommap.MAP_SHARED,                  // Updates to the mapping are visible to other processes mapping the same region
	); err != nil {
		return nil, err
	}
	return idx, nil
